...
st.markdown(
    ...
    La arquitectura fue reorganizada para separar:
    - `backend/`: API FastAPI
    - `frontend/`: dashboard Streamlit
    - `src/`: lógica financiera original
    - `report/`: documento LaTeX del informe
    """
)
...
with st.sidebar:
    ...
    st.info(
        "Los módulos se encuentran en `frontend/pages/`. "
        "El cliente HTTP sugerido quedó en `frontend/api_client.py`."
    )
...
st.markdown(
    ...
    - **09 Panel de decisión:** consolidado final del proyecto.
    """
)
